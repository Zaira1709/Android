...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        cv.put(IMAGEN,R.drawable.c6);
        cv.put(NIVEL,2);
        cv.put(LATITUD,19.6533755);
        cv.put(LONGITUD,-99.2147361);
        db.insert(TABLA_ANIMAL,NOMBRE,cv);
    ...
    public ArrayList<String> getRecord(int gano){
        ArrayList<String> record = new ArrayList<>();
            ...
            SQLiteDatabase db = this.getReadableDatabase();
            try {
                cursor = db.rawQuery("SELECT "+NOMBRE+" FROM "+TABLA_RECORD+" INNER JOIN "+TABLA_ANIMAL+" ON "+TABLA_RECORD+"."+ID_ANIMAL+"= "+TABLA_ANIMAL+ "."+ID+" WHERE "+GANO+" = "+gano
                        ,null);
                if (cursor!=null && cursor.getCount()>0){
                    cursor.moveToFirst();
                    do {
                        record.add(String.valueOf(cursor.getString(0)));
                    }
                    while (cursor.moveToNext());
                }
